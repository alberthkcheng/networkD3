...
sankeyNetwork <- function(Links, Nodes, Source, Target, Value, 
    ...
    height = NULL, width = NULL, iterations = 32, sinksRight = TRUE, xPos = NULL)
{
    ...
    # add node breadth if specified
    if (is.character(xPos)) {
    ...
    options = list(NodeID = NodeID, NodeGroup = NodeGroup, LinkGroup = LinkGroup, 
        ...
        margin = margin, sinksRight = sinksRight, iterations = iterations)
